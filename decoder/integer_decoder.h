...
#ifndef GX_INTEGER_DECODER_H
#define GX_INTEGER_DECODER_H
...
#include "integer_decoder_args.h"
#include "../core/gx_exception.h"

namespace gx
{
    template<typename T, typename V>
    struct integer_decoder : decoder<T, V>
    {
        integer_decoder(const std::vector<feature_info> &features, int_fitness_evaluator_delegate fitness_evaluator,
                        int_batch_fitness_evaluator_delegate batch_fitness_evaluator) noexcept;
    ...
    private:
        ...
        int_fitness_evaluator_delegate fitness_evaluator_;
        int_batch_fitness_evaluator_delegate batch_fitness_evaluator_;
        size_t features_count_;
        size_t total_bits_;
    };

    template<typename T, typename V>
    integer_decoder<T, V>::integer_decoder(const std::vector<feature_info> &features,
    ...
    template<typename T, typename V>
    float integer_decoder<T, V>::fitness_value(T *gene, feature* decoded_features) const
    {
        std::vector<int> features(features_count_);
        for (size_t i = 0; i < features_count_; ++i)
        {
            features[i] = feature_decoders_[i].decodei(gene);
            decoded_features[i].int_value = features[i];
    ...
    template<typename T, typename V>
    void integer_decoder<T, V>::batch_fitness_value(T *gene, size_t batch_size, float* result, feature* decoded_features) const
    {
        std::vector<int> batch_features(features_count_ * batch_size);
        for (size_t j = 0; j < batch_size; ++j)
            for (size_t i = 0; i < features_count_; ++i)
            {
                batch_features[j * features_count_ + i] = feature_decoders_[i].decodei(gene);
                decoded_features[j * features_count_ + i].int_value = batch_features[j * features_count_ + i];
    ...
    template<typename T, typename V>
    void integer_decoder<T, V>::decode(T *gene, feature *result) const
    {
        for (size_t i = 0; i < features_count_; ++i)
        {
    ...
    template<typename T, typename V>
    size_t integer_decoder<T, V>::total_bit_count() const noexcept
    {
        return total_bits_;
    }

    template<typename T, typename V>
    size_t integer_decoder<T, V>::feature_count() const noexcept
    {
        return features_count_;
    }

    template<typename T, typename V>
    bool integer_decoder<T, V>::register_type() noexcept
    {
        return decoder_factory<T, V>::instance().register_creator(decoder_type::integer_decoder, [](const decoder_info &info)
        {
            auto args = static_cast<integer_decoder_args*>(info.args);
            if(!args)
            ...
            return std::make_unique<integer_decoder<T, V>>(
                    features,
